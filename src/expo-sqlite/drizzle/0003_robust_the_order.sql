...
CREATE TABLE `__new_movie_details` (
	...
	`last_updated` integer DEFAULT 1759876587 NOT NULL
);
...
CREATE TABLE `__new_movie_reviews` (
...
--> statement-breakpoint
INSERT INTO `__new_movie_reviews`("id", "author", "content", "created_at", "url", "movie_id") SELECT "id", "author", "content", "created_at", "url", "movie_id" FROM `movie_reviews`;--> statement-breakpoint
DROP TABLE `movie_reviews`;--> statement-breakpoint
ALTER TABLE `__new_movie_reviews` RENAME TO `movie_reviews`;--> statement-breakpoint
